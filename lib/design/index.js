...
internals.definition = {
    register: function(server, options, next)
    {
        ...
        // override options
        options.stylesPath = internals.designCfg.stylesPath;
        options.scriptsPath = internals.designCfg.scriptsPath;
        options.fontsPath = internals.designCfg.fontsPath;
        options.assetsPath = internals.designCfg.assetsPath;
        options.faviconPath = internals.designCfg.faviconPath;
        options.viewsPath = internals.designCfg.viewsPath;
        options.homeEndPoint = internals.routeCfg.apiPath + internals.routeCfg.apiVersion + internals.routeCfg.endPoints.home;
        if (CfgManager.get('env') === 'development')
        {
            options.bowerPath = internals.designCfg.bowerPath;
        }
        ...
        var configView = {
            ...
            path: Path.join(options.dirname, internals.designCfg.viewsPath),
        ...
        try
        {
            var partials = Fs.readdirSync(Path.join(options.dirname, internals.designCfg.partialsPath));

            if (partials)
            {
                configView.partialsPath = Path.join(options.dirname, internals.designCfg.partialsPath);
            }
        }
        catch(e)
        {
        ...
        try
        {
            var layout = Fs.readdirSync(Path.join(options.dirname, internals.designCfg.layoutPath));

            if (layout)
            {
                configView.layoutPath = Path.join(options.dirname, internals.designCfg.layoutPath);
